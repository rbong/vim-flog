...
for commit_index, commit in ipairs(commits) do
  ...
  -- Calculate whether certain lines should be printed

  local should_print_merge = graph_opt and (nparents > 1
    or moved_parent_branch_index
    or (nparents == 0 and nbranches == 0)
    or (nparents == 1 and branch_indexes[parents[1]] ~= commit_branch_index))
  local should_print_complex = should_print_merge and ncomplex_merges > 0
  local should_print_missing_parents = nmissing_parents > 0

  -- Calculate total lines out

  local total_lines = (ncommit_lines
    + (should_print_merge and 1 or 0)
    + (should_print_complex and 1 or 0)
    + (should_print_missing_parents and 2 or 0))
  ...
  if should_print_merge then
    for _, str in ipairs(merge_line) do
      io.write(str)
    end
    io.write('\n')

    if should_print_complex then
  ...
  if should_print_missing_parents then
